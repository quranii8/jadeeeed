...
import { ArrowRight, MapPin, Clock, Users, Lightbulb, BookOpen, Heart, Sparkles, Star, GitBranch } from "lucide-react";
    ...
    <article className="max-w-4xl mx-auto px-5 py-8">
      ...
      {prophet.nasab && (
        <div className="card p-6 md:p-8 mb-5" style={{ borderRight: "4px solid #8b5cf6" }}>
          ...
          <p className="text-base leading-loose">{prophet.nasab}</p>
        </div>
      )}
      ...
      {prophet.earlyLife && (
        <div className="card p-6 md:p-8 mb-5">
          ...
          <p className="text-base leading-loose">{prophet.earlyLife}</p>
        </div>
      )}

      {/* تفاصيل المهمة */}
      {prophet.missionDetails && (
        <div className="card p-6 md:p-8 mb-5">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: "#16a34a" }}>
            <BookOpen className="w-6 h-6" />
            تفاصيل رسالته ومهمته
          </h2>
          <p className="text-base leading-loose">{prophet.missionDetails}</p>
        </div>
      )}

      {/* مراحل القصة */}
      {prophet.storyStages && prophet.storyStages.length > 0 && (
        <div className="card p-6 md:p-8 mb-5">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: "var(--primary)" }}>
            <BookOpen className="w-6 h-6" />
            مراحل القصة
          ...
          <div className="space-y-4">
            {prophet.storyStages.map((stage, i) => (
              <div key={i} className="p-4 rounded-xl" style={{ background: "var(--bg-soft)" }}>
                <div className="flex items-start gap-3">
                  <span
                    className="flex-shrink-0 w-8 h-8 rounded-full grid place-items-center text-sm font-bold"
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-relaxed">{stage.content}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* الآيات القرآنية */}
      {prophet.keyVerses && prophet.keyVerses.length > 0 && (
        <div className="card p-6 md:p-8 mb-5" style={{ borderRight: "4px solid #ec4899" }}>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: "#ec4899" }}>
            <BookOpen className="w-6 h-6" />
            الآيات القرآنية المرتبطة به
          </h2>
          <div className="space-y-4">
            {prophet.keyVerses.map((v, i) => (
              <div key={i} className="p-5 rounded-xl" style={{ background: "var(--bg-soft)" }}>
                <div
                  className="font-quran text-2xl md:text-3xl leading-loose text-center my-3"
      ...
      {prophet.wisdomSayings && prophet.wisdomSayings.length > 0 && (
        <div className="card p-6 md:p-8 mb-5" style={{ borderRight: "4px solid #06b6d4" }}>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: "#06b6d4" }}>
            <Lightbulb className="w-6 h-6" />
            الحكم والأقوال المأثورة
          </h2>
          <div className="space-y-3">
            {prophet.wisdomSayings.map((w, i) => (
              <div key={i} className="p-4 rounded-xl flex items-start gap-3" style={{ background: "var(--bg-soft)" }}>
                <span className="text-xl">💎</span>
                <p className="text-sm leading-relaxed flex-1">{w}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* الأنبياء ذوو الصلة */}
      {prophet.relatedProphets && prophet.relatedProphets.length > 0 && (
        <div className="card p-6 md:p-8 mb-5">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: "var(--primary)" }}>
            <Users className="w-6 h-6" />
            أنبياء ذوو صلة بقصته
          </h2>
          <div className="flex flex-wrap gap-2">
            {prophet.relatedProphets.map((rp) => {
              const p = getProphet(rp);
              if (!p) return null;
              return (
                <Link
                  key={rp}
